      ...
      if (!err && settings) {
         ...
         if (status == "failed") {
            if (failed.length > 0) {
                  ...
                  if (data) {
                     if (preFailed.length > 0) {
                        app.checkPrefailed(data, (newData) => {
                           if (newData) {
                              app.pushToArray(preFailed, newData);
                           }
                        });
                     } else {
                        app.pushToArray(preFailed, data);
                     }
                  }
               });
            } else if (failed.length <= 0 && preFailed.length > 0) {
               ...
               app.pushToArray(preFailed, payload);
            }
         } else if (status === "passed") {
            app.checkPassed(payload, (data) => {
               if (data.length > 0) {
                  app.pushToArray(passed, data);
               }
            });
         } else {
...
app.pushToArray = (arry, payload) => {
   _bot.checkAppSettings('app', (err, settings) => {
      if (!err && settings) {
         ...
         payload.forEach((load) => {
            arry.push(load);
            load.type = arry + ' Process';
            _bot.writeError('processes', arry + 'Process', load);
         });
         if(settings.debugging == 'true'){
         ...
         err.payload = payload;
         err.internalMessage = "Issue with pushing to " + arry + "! no settings.";
         _bot.writeError('internal', 'pushToArray', err);
      }
   });
};

app.pushToPassed = (payload) => {
   _bot.checkAppSettings('app', (err, settings) => {
      if (!err && settings) {
         if (settings.debugging == 'true') {
            console.log("Starting to push to passed at: " + _bot.serverTime());
         }
         payload.forEach((load) => {
            passed.push(load);
            load.type = 'Passing Process';
            _bot.writeError('processes', 'PassingProcess', load);
         });
         if(settings.debugging == 'true'){
            console.log("Finished pushing to preFailed at: " + _bot.serverTime());
         }
         return preFailed;
      } else {
         err.payload = payload;
         err.internalMessage = "Issue with pushing to prefield! no settings.";
         _bot.writeError('internal', 'pushToPrefailed', err);
      }
   });
};
...
app.init = () => {
   ...
   _bot.checkAppSettings('app', (err, data) => {
      if (!err && data) {
         ...
         setTimeout(() => {
            console.log("Nothing found, will check again!");
            app.init();
         }, 5000);
      }
   });
