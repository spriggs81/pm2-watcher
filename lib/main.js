   ...
   _bot.checkAppSettings('app', (err, settings) => {
      if (!err && settings) {
         ...
         if (status == "failed") {
            ...
            if(failed.length > 0 && failed[0].name === "No Processes are online!"){
               failed = [];
   ...
   _bot.checkAppSettings('app', (err, settings) => {
      if (!err && settings) {
         ...
         if (preFailed.length > 0) {
            ...
            _bot.checkHoldingTime(preFailed,maxTime,(data) => {
               if(data){
                  data.forEach((process) => {
                     if (settings && settings.debugging === 'true') {
                        console.log("Creating message, pushing issue into Failed array, and removing issue from prefailed array @ " + _bot.serverTime());
                     }
                     if(process.pid =='none' && process.id == '911'){
                        failed = [];
                        preFailed = [];
                     }
                     failed.push(process);
                     _bot.findArray(preFailed,process.id,(index) => {
                        preFailed.splice(index, 1);
                     });
                  });
                  // TODO: ================ Remove This After testing =======================
                  console.log("(testing) - requested message be created to be emailed @ " + _bot.serverTime() + " for found problem(s)");
      ...
      if(!err && settings){
         ...
         if(passed.length > 0){
            passed.forEach((item) => {
               ...
               console.log("Seconds Left: ",Math.round((30000 - pastTime) / 1000)," before confirming no problem with ID: ",item.id," / name: ",item.name);
               if(pastTime > 30000){
                  if(preFailed.length > 0){
         ...
         if(problemFree.length > 0){
            // TODO: Remove this after testing
            console.log("(testing) - Sending request to create email @ " + _bot.serverTime()," for corrected problem(s)");
