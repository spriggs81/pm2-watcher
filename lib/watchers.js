   ...
   _bot.checkAppSettings('app', (err, settings) => {
      if (!err && settings) {
         ...
         if (array.checkLeng('preFailed') === true) {
            array.checkHoldingTime('preFailed',maxTime,'failed',Date.now(),(data) => {
               if(data){
                  ...
                  array.lastCheckBeforeEmail(data,(newData) => {
                     if(newData){
                        array.pushToArray('failed',newData,'watchPrefailed','failed');
                        mailer.createMessage('failed', newData);
                     }
                  });
               }
            });
   ...
   _bot.checkAppSettings('app',(err,settings) => {
      if(!err && settings){
         ...
         if(array.checkLeng('passed') == true){
            ...
            array.checkHoldingTime('passed',maxTime,'passed',Date.now(),(data) => {
               if(data){``
                  data.forEach((process) => {
                     if(array.checkLeng('preFailed') == true){
