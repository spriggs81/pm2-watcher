...
app.checkHoldingTime = (array,max,status,theTime,cb) => {
   let data = [];
   app[array].forEach((process) => {
      // checking the age of the failed process
      const aged = theTime - process.checked;
      const timeStatus = status == "passed" ? "before confirming no problem with ID:" : " before email sent due to problem with Process ID:";
      console.log("(testing) - Seconds Left: ",Math.round((max - aged) / 1000),timeStatus,process.id+" / NAME: "+process.name+"!");
...
app.lastCheckBeforeEmail = (payload,cb) => {
   let data = [];
   app['preFailed'].forEach((load) => {
      payload.forEach((pre) => {
         if(load.id == pre.id){
            if(pre.mailed === false){
               pre.mailed = true;
               data.push(load);
               app.removePayload(pre);
            }
         }
      });
   });
...
app.removePayload = (item) => {
   for(let i = 0; i < app['preFailed'].length; i++){
         if(app['preFailed'][i].id == item.id){
            app['preFailed'].splice(i,1);
         }
   }
}
