...
app.checkAppSettings = (dir,cb) => {
   _data.open(dir,'settings',(err,data) => {
      if(!err && data){
         ...
         const fD = typeof(Number(data.failedDelays)) == 'number' && Number(data.failedDelays) > 0 ? Number(data.failedDelays) : false;
         const pD = typeof(Number(data.passedDelays)) == 'number' && Number(data.passedDelays) > 0 ? Number(data.passedDelays) : false;
         if(hn && pt && se && fm && sr && ur && pd && bg && fD && pD){
            cb(false,data);
         } else {
...
app.createLogs = (place,name,data) => {
   ...
   _logs.append(place,fixedname,str,(err) => {
      if(!err){
         console.log(place+" log has been updated!");
      } else {
