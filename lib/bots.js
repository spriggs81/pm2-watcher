...
app.checkHoldingTime = (array,max,cb) => {
   let data = [];
   array.forEach((process) => {
      // checking the age of the failed process
      const aged = Date.now() - process.checked;
      // TODO: ================ Remove This After testing =======================
      console.log("Seconds Left: ",Math.round((max - aged) / 1000)," before email sent due to problem with Process ID:"+process.id+" / name: "+process.name+"!");
      if (process.id === "911" && process.status === "offline" || (aged > max)) {
